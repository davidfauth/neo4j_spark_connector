...
object Neo4jDataFrame {
    ...
    def mergeEdgeList(sc: SparkContext,
      ...
      val mergeStatement = s"""
        UNWIND {rows} as row
        MERGE (source:`${source._1}` {`${sourceLabel}` : row.source.`${sourceLabel}`}) 
        ON CREATE SET source += row.source
        MERGE (target:`${target._1}` {`${targetLabel}` : row.target.`${targetLabel}`}) 
        ON CREATE SET target += row.target
  ...
  def createNodes(sc: SparkContext, dataFrame: DataFrame, nodes: (String,Seq[String]), renamedColumns: Map[String,String] = Map.empty): Unit = {
    ...
    val createStatement = s"""
        UNWIND {rows} as row
        CREATE (node:`${nodes._1}` {`${nodeLabel}` : row.node_properties.`${nodeLabel}`})
  ...
  def mergeNodes(sc: SparkContext, dataFrame: DataFrame, nodes: (String,Seq[String]), renamedColumns: Map[String,String] = Map.empty): Unit = {
    ...
    val createStatement = s"""
        UNWIND {rows} as row
        MERGE (node:`${nodes._1}` {`${nodeLabel}` : row.node_properties.`${nodeLabel}`})
